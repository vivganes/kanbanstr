...
function createDonationStore() {
  const { subscribe, set, update } = writable<DonationState>({
    showPopup: false,
    lastIgnored: isBrowser ? Number(localStorage.getItem('lastDonationIgnored')) : null
  });

  return {
    subscribe,
    showDonationRequest: () => {
      const lastIgnored = isBrowser ? Number(localStorage.getItem('lastDonationIgnored')) : null;
    ...
    ignoreDonation: () => {
      ...
      if (isBrowser) {
        localStorage.setItem('lastDonationIgnored', timestamp.toString());
      }
