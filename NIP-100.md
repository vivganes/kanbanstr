...
#### Card Comment Events (kind: 30303)
- `content`: The comment text (stored in the event content field, not in a tag)
- `e` (card reference): `['e', '<card-event-id>', 'card']` — references the card being commented on
- `a` (card a-tag): `['a', '30302:<boardPubkey>:<cardDTag>']` — binds the comment to a specific card; used for filtering
- `e` (reply reference, optional): `['e', '<parent-comment-event-id>', 'reply']` — present only when replying to another comment
- `p` (optional): mentions of users referenced in the comment

Comments are fetched by filtering on `kinds: [30303]` with `#a: ['30302:<boardPubkey>:<cardDTag>']`. Top-level comments have no `e` tag with marker `'reply'`; replies include one with marker `'reply'` pointing to the parent comment event. Card comment events are **not replaceable** — each comment is a distinct immutable event.
